...
namespace LEPInstaller
{
    public partial class Form1 : Form
    {
        ...
        public Form1(int CmdMode=0)
        {
            ...
            // https://github.com/julixian/LocaleEmulatorPlus/issues/22.
        ...
        private void Form1_Load(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show(this, "Sorry, Locale Emulator Plus is only for Windows 7, 8/8.1 and 10.",
                    "OS Outdated",
                    MessageBoxButtons.OK,
                    MessageBoxIcon.Error);
        ...
        {
            Process.Start(Path.Combine(crtDir, "LEPGUI.exe"));
            Application.Exit();
        }
        ...
        private void buttonUninstall_Click(object sender, EventArgs e)
        {
            ...
            MessageBox.Show(this, "Uninstall finished. Thanks for using Locale Emulator Plus :)\r\n" +
                            "\r\n" +
        ...
        private void DoRegister(bool allUsers)
        {
            try
            {
                ...
                rs.RegisterAssembly(Assembly.LoadFrom(Path.Combine(crtDir, @"LEPContextMenuHandler.dll")),
        ...
        private void DoUnRegister(bool allUsers)
        {
            try
            {
                ...
                var rs = new RegistrationServices();
                rs.UnregisterAssembly(Assembly.LoadFrom(Path.Combine(crtDir, @"LEPContextMenuHandler.dll")));
        ...
        {
            var dllPath1 = Path.Combine(crtDir, @"LEPContextMenuHandler.dll");
            var dllPath2 = Path.Combine(crtDir, @"LEPCommonLibrary.dll");
            ...
            try
            {
                File.WriteAllBytes(dllPath1, Resources.LEPContextMenuHandler);
                File.WriteAllBytes(dllPath2, Resources.LEPCommonLibrary);
        ...
        // We should not use the LEPCommonLibrary.
        private static string GetLEPVersion()
        {
            try
            {
                var versionPath =
