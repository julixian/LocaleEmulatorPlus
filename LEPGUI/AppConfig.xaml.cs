...
namespace LEPGUI
{
    ...
    public partial class AppConfig
    {
        ...
        public AppConfig()
        {
            ...
            Title += Path.GetFileName(App.StandaloneFilePath).Replace(".lep.config", "");
        ...
        {
            var crt = new LEPProfile(Path.GetFileName(App.StandaloneFilePath),
            ...
            LEPConfig.SaveApplicationConfigFile(App.StandaloneFilePath, crt);
        }
        ...
        private void CreateShortcut(string path)
        {
            try
            {
                ...
                link.SetPath(GlobalHelper.GetLEPProcPathForTarget(path));
                ...
                link.SetDescription($"Run {Path.GetFileName(path)} with Locale Emulator Plus");
        ...
        {
            //Run the application.
            var targetPath = App.StandaloneFilePath.Replace(".lep.config", "");
            Process.Start(GlobalHelper.GetLEPProcPathForTarget(targetPath),
                          $"-run \"{targetPath}\"");
        ...
        private void bShortcut_Click(object sender, RoutedEventArgs e)
        {
            ...
            CreateShortcut(App.StandaloneFilePath.Replace(".lep.config", ""));
