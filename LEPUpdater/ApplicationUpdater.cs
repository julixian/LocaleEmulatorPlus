...
namespace LEPUpdater
{
    internal static class ApplicationUpdater
    {
        private const string UpdateInfoUrl =
            "https://raw.githubusercontent.com/julixian/LocaleEmulatorPlus/master/VersionInfo.xml";
        ...
        internal static void CheckApplicationUpdate(string version, NotifyIcon notifyIcon)
        {
            try
            {
                ...
                var stream = client.DownloadDataStream(UpdateInfoUrl);
        ...
        private static void ProcessUpdate(XmlDocument xmlContent, NotifyIcon notifyIcon)
        {
            ...
            var crtVer = GlobalHelper.GetLEPVersion();
            ...
            if (CompareVersion(crtVer, newVer))
            {
                try
                {
                    ...
                    notifyIcon.ShowBalloonTip(0,
                                              $"New Version {version} Available (Current: {GlobalHelper.GetLEPVersion()})",
